...
import { BUILDING_SPECS, UPGRADE_SPECS, TX_SCALE } from "@/types/game";
...
function prodStats(spec: BuildingSpec | UpgradeSpec): { icon: React.ReactElement; value: string; color: string }[] {
  ...
  if (spec.txProduction > 0)
    items.push({ icon: <SwapHorizRounded sx={{ fontSize: IC }} />, value: `+${(spec.txProduction / TX_SCALE).toFixed(1)}/s`, color: "#4ADE80" });
